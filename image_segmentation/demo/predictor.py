...
class VisualizationDemo(object):
    ...
    def run_on_image(self, image, task):
        ...
        assert task in ['panoptic', 'semantic', 'instances']
        alpha = 1.0
        ...
        if task == 'panoptic':
            ...
            panoptic_seg.to(self.cpu_device), segments_info, alpha=alpha
        )
            ...
            predictions = self.predictor(image, task)
            #print(predictions["sem_seg"].shape, predictions["sem_seg"].argmax(dim=0).to(self.cpu_device))
            #print(self.metadata) #.stuff_classes)
            vis_output['semantic_inference'] = visualizer.draw_sem_seg(
                predictions["sem_seg"].argmax(dim=0).to(self.cpu_device), alpha=alpha
            )
            ...
            vis_output['instance_inference'] = visualizer.draw_instance_predictions(predictions=instances, alpha=alpha)
